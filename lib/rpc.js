...
class RpcHandler extends Handler {
  constructor(session, socket) {
    super()
    //websock with session, status manage be easier.
  ...
  async load() {
    const { socket } = this
    const source = await readFile(require.resolve('../agent/app.bundle.js'), 'utf8')
  ...
  async modules() {
    //const appmodules = await this.session.enumerateModules()
    //console.info(appmodules)
...
class SpringBoardHandler extends Handler {
  ...
  async load() {
    const { socket } = this
    const source = await readFile(require.resolve('../agent/springboard.bundle.js'), 'utf8')
    const script = await this.session.createScript(source)
