...
channels.session.on('attach', async(ctx, { device, bundle }) => {
  ...
  try {
    ...
    session = app.pid ? await dev.attach(app.name) :
      await FridaUtil.spawn(dev, app)
  ...
  socket.socket.on('classes', async ({ needle }, ack) => {
    // needle: search keyword
    let classes = cache.fetch('classes', agent.classes().bind(agent))
    ack(classes)
  }).on('methods', async ({ clz }, ack) => {
    let methods = cache.fetch(`methods_${clz}`, () => agent.methods(clz))
    ack(methods)
